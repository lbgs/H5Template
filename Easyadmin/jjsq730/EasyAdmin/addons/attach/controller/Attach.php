		...
		if(empty($m)){
			...
			if($param['zuid']!=$param['uid']){
				...
				if($point<$param['score']&&$param['score']>0){
					return array('code'=>0,'msg'=>'积分不足');
				}else{
					
					if($param['score']>0){
						//Db::name('user')->where('id',$param['uid'])->setDec('point',$param['score']);
					point_note(0-$param['score'],$param['uid'],'attachdownload');
					point_note($param['score'],$param['zuid'],'attachdownload');
		...
		if($config['open']==1){
		 ...
		 if(empty($mmn)){
		 	...
		 	if($res>0){
		 		...
		 		return array('code'=>200,'msg'=>'上传成功','ext'=>$data['ext'],'id'=>$res,'path'=>$data['savepath'],'md5'=>$data['md5'],'savename'=>$data['savename'],'filename'=>$data['name'],'info'=>$info);
		 	}else{
		 		return array('code'=>0,'msg'=>'上传失败');
		 	}
		 }else{
		 	return array('code'=>200,'msg'=>'上传成功','ext'=>$mmn['ext'],'id'=>$mmn['id'],'path'=>$mmn['savepath'],'md5'=>$mmn['md5'],'savename'=>$mmn['savename'],'filename'=>$mmn['name'],'info'=>$mmn);
		 }
		    }else{
			...
			if(empty($n)){
					...
					if($info){
						...
						if($res>0){
							...
							return array('code'=>200,'msg'=>'上传成功','ext'=>$data['ext'],'id'=>$res,'path'=>$path,'headpath'=>$realpath,'md5'=>$data['md5'],'savename'=>$data['savename'],'filename'=>$data['name'],'info'=>$info->getInfo());
						}else{
							return array('code'=>0,'msg'=>'上传失败');
						}
				...
				$realpath=WEB_URL.$path;
				return  array('code'=>200,'msg'=>'上传成功','ext'=>$n['ext'],'id'=>$n['id'],'path'=>$path,'headpath'=>$realpath,'md5'=>$n['md5'],'savename'=>$n['savename'],'filename'=>$n['name'],'info'=>$n);
			}
		}
