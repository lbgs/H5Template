 ...
 public function index($keyword = '', $page = 1)
    {
        ...
        $user_list = $this->forum_model->alias('f')->join('forumcate c', 'c.id=f.tid')->field('f.*,c.id as cid,c.name')->order('f.id desc')->where($map)->paginate(10);
    ...
    public function update()
    {
        if ($this->request->isPost()) {
           ...
           $data['content']= remove_xss($data['content']);
            if ($this->forum_model->update($data) !== false) {
                return json(array('code' =>200, 'msg' => '更新成功'));
            } else {
                return json(array('code' => 0, 'msg' => '更新失败'));
            }
